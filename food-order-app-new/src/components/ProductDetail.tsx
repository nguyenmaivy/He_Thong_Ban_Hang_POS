...
interface ProductDetailProps {
    product: Product;
    onClose: () => void;
    addToCart: (product: Product & { quantity: number }) => void; // Thêm type cho quantity
}

const ProductDetail: React.FC<ProductDetailProps> = ({ product, onClose, addToCart }) => {
    const [quantity, setQuantity] = useState<number>(1);
    const [sideDishes, setSideDishes] = useState<string[]>(["Vegetables"]); // Mặc định chọn Vegetables
                    ...
                    <div className={styles.productDetails}>
                        ...
                        <div className={styles.addToCartContainer}>
                            <button className={styles.addToCartButton} onClick={handleAddToCart}>
                                <FaShoppingCart className={styles.cartIcon} /> {/* Xóa onClick ở đây */}
